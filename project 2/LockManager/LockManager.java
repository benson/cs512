            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                    ...
                    if(dataObj2.getLockType() == DataObj.WRITE)
                    {
                        ...
                        // if size > 1, multiple people have locks, which means multiple read locks, which means we can't convert 
                        if(size > 1)
                            return true;
                        // size == 1, so there is no conflict (changing this read lock to a write lock)
                        bitset.set(0, true);
                        return false;
                    }
                }
